...
```
第一个位置： `$System_Config[‘f2fpay_app_id’]=”` 这个是支付宝商家平台里的`APPID`，开通收款码服务后进入支付宝商家平台签约管理，查看`PID`和`Key` https://openhome.alipay.com/platform/keyManage.htm 有一个基础应用，就是那个`APPID`
